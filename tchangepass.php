...
<div class="container" >
	  ...
	  <div align="center">
                        		<a href="tprofile.php"><button type="button" class="btn btn-default btn-lg">
  <span class="glyphicon glyphicon-home" aria-hidden="true"></span> Home
		...
		<div class="panel panel-danger" style="width:600px; margin-left: 200px" >
			...
			<div class="panel-body">
				...
				if(isset($_POST['submit']))
				{
					...
					$npass=$_POST['new'];
					$opass=$_POST['old'];
					$select="select * from t_details where Password='$opass' and Username='".$_SESSION['user']."'";
					$ress=mysql_query($select);
					$recs=mysql_fetch_row($ress);
					if($recs)
					{
					$select="select * from t_details where Password='$opass' and Username='".$_SESSION['user']."'";
						$cp="UPDATE t_details SET Password='$npass' where Password='$opass' and Username='".$_SESSION['user']."'";
						//echo $cp;
						$res=mysql_query($cp);
						if($res)
						{
		...
		//echo $msg;
				...
				<form role="form" action="tchangepass.php" method="post" id="cp" onsubmit="return cp('cp');">
	  		...
	  		<label class="col-md-4 control-label" for="Farme password">New Password</label>  
	 			 ...
	 			 <div class="form-group" style="padding-bottom:40px; pa">
	  		<label class="col-md-4 control-label" for="Farme password"> Confirm Password</label>  
	  			<div class="col-md-6">
	 			 <input  type="Password" id="cnew" name="cnew" placeholder=" Re-Enter Password" class="form-control"  onchange="check(this)" required>
	 			 </div>
	 			 </div>			
